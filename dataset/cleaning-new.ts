...
function cleanData(data: FinalData): ActuallyFinal {
  const tags = data.menuTag.map((e) => e.trim().toLocaleLowerCase());
  const keywords = data.associatedKeywords.map((e) =>
    e.trim().toLocaleLowerCase()
  );
  const dishType = data.dishType.map((e) => e.trim().toLocaleLowerCase());
...
(async function main() {
  const raw = await fs.readFile("additional.csv", { encoding: "utf-8" });
  ...
  await fs.writeFile("additonal-cleaned.json", JSON.stringify(data), {
    encoding: "utf-8",
  });
})();
